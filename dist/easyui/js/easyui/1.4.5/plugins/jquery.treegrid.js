...
/*easyui集成mCustomScrollbar*/
/*$(_f0).html(_f5.join(""));*/
if($(_f0).hasClass("mCustomScrollbar")){
    $(_f0).find(".mCSB_container").html(_f5.join(""));
}else{
    $(_f0).html(_f5.join(""));
}
            ...
            return jq.each(function(){
                ...
                nodes.easyui_draggable({
                    ...
                    proxy: function(source){
						...
						var dragHtml =  $('.mcc-mt-list').find('tr[node-id="' + row.id + '"]').clone().removeAttr('id').removeClass('droppable').css({'background':'#3e9bd0'});
						//p.html('<span>&nbsp;</span>'+ row[opts.treeField]);
						//p.html('<span>&nbsp;</span>');
						dragHtml.find("td:first").css({'display':'none'});
						p.append(dragHtml);
						p.hide();
						return p;
					},
                    ...
                    onBeforeDrag:function(e){
                        ...
                        $(this).next('tr.treegrid-tr-tree').find('tr[node-id]').easyui_droppable({accept:'no-accept'});
...
//						e.data.offsetHeight = 0;
                    ...
                    onStartDrag:function(){
                        $(this).easyui_draggable('proxy').css({
                            left:-10000,
                            top:-10000
                        });
                        var row = getRow(this);
                    ...
                    onDrag:function(e){
                        ...
                        if (d>3){	// when drag a little distance, show the proxy object
                            $(this).easyui_draggable('proxy').show();
                            var tr = opts.finder.getTr(target, $(this).attr('node-id'));
                            var treeTitle = tr.find('span.tree-title');
                            e.data.startX = treeTitle.offset().left;
                            e.data.startY = treeTitle.offset().top;
                    ...
                    onStopDrag:function(){
                        $(this).next('tr.treegrid-tr-tree').find('tr[node-id]').easyui_droppable({accept:'tr[node-id]'});
                        for(var i=0; i<state.disabledNodes.length; i++){
                            var tr = opts.finder.getTr(target, state.disabledNodes[i]);
                            tr.easyui_droppable('enable');
                        }
                        state.disabledNodes = [];
                        var row = t.treegrid('find', state.draggingNodeId);
                        $(this).data("data",row);
						$(this).data("mtid",row.mtid);
						$(this).data("type",row.accountType);
                ...
                function allowDrop(source, allowed){
                    var icon = $(source).easyui_draggable('proxy').find('span.tree-dnd-icon');
